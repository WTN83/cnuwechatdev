...
class WechatRequest{
    ...
    public static function location(&$request){
        // $content = IntelligentInterface::semanticSemproxy($query='查一下明天从北京到上海的南航机票',$category='flight,hotel',$request['fromusername'],$request['location_x'],$request['location_y']);
        $content = IntelligentInterface::semanticSemproxy($query='查一下明天从北京到上海的南航机票',$category='flight,hotel',$request['fromusername'], $city='北京');
    ...
    public static function eventLocation(&$request){
        $content = '纬度坐标:'.$request['latitude'].'-经度坐标:'.$request['longitude'];
        return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);
    }
